...
把嫌犯押到拘留所登記窗口時要輸入條號，答不出來會走到結局訊息
「這場遊戲維護法律，夥伴。找到你的文件後再回來吧。」ScummVM 沒有、也不適合提供
略過選項——那等於把遊戲主要玩法拿掉。

條號原本印在原版包裝內的說明書上。為了讓沒有實體手冊的人也能把遊戲跑完，
本專案附了一份中譯對照：**[docs/中文條號速查.md](docs/中文條號速查.md)**
（刑法條號、車輛法規條號、無線電代碼），交付包裡也有同一份。遊戲內幾句提示代碼的
對白後面另外標了指路，玩到那裡不用回頭翻文件。
...
唯一的例外是 VGA 版的條號表。它是查手冊型防拷，沒有條號就玩不完，而且條號本身是
說明書上的參考資料、不是遊戲程式碼或美術，所以中譯版隨包附上（見上面的
[條號速查](#install)）。這是刻意的取捨，不是漏掃。
