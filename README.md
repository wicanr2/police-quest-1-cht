...
通報案件要報刑法條號、開單要報車輛法規條號，答不出來會走到結局訊息
「這場遊戲維護法律，夥伴。找到你的文件後再回來吧。」ScummVM 沒有、也不適合提供
略過選項——那等於把遊戲主要玩法拿掉。條號表在原版包裝內的說明書裡，本專案不附。

原版 AGI 則沒有這種檢查。ScummVM 的 AGI 引擎確實有 `copy_protection` 選項，但只作用於
《Gold Rush!》；偵測表裡 PQ1 的 DOS 條目用的是不帶防拷旗標的那組。無線電通報在 AGI 版
是由 parser 動詞觸發、遊戲自己組句子，玩家不需要輸入條號。
